...
10. [Startup Race Quick Fix Plan](design/10_startup-race-quick-fix-plan.md) — implemented in M2_017. Hướng sửa nhanh cho lỗi race ở M2_016 (cửa sổ Tauri load Gateway URL trước khi Gateway sẵn sàng): đảo thứ tự trong lib.rs setup() — chỉ tạo cửa sổ sau khi start_gateway() xong. Đánh đổi: cửa sổ không hiện gì trong lúc chờ (tới 12s).
...
12. [Voice Selection: API vs DOM, và Gap Analysis Tính Năng](design/12_voice-selection-api-vs-dom-and-feature-gaps.md) — RESEARCH (2026-09-22). Khuyến nghị chọn giọng bằng API, không DOM (Vbee có catalog API `GET /api/v1/voices` + SDK; DOM M2_014 không có slug→row mapping và có bug click bị dialog chặn; `GET /api/public/v1/voices` test live bị 401 — lane khả thi nhất là studio session-token như M2_019). Đề xuất 6-bước: capture endpoint catalog studio → `getVoices()` trong page context → `GET /api/voices` → UI picker → validate `voice_code` khi queue → lane official M4. Kèm bảng gap analysis 15 tính năng thiếu sót map roadmap M3–M10 + MVP cut line.
...
17. [Startup Race Quick Fix: Implemented And Live-Verified](milestones/M2_017_startup-race-quick-fix-implemented.md) — implements design/10 exactly as planned (user chose it over design/11's splash-screen approach): reorders `lib.rs`'s `setup()` so `start_gateway()`/`start_browser()` run and succeed *before* the window is created, instead of racing it. Live timing test (launch + poll window-visibility and `/health` every 200ms from t=0, in one script to avoid cross-call measurement gaps) confirms zero samples where the window was visible while the Gateway was still unhealthy — Gateway healthy at t=+2.3s, window appears at t=+4.8s, never before. Trade-off predicted by design/10 is now confirmed live: no more error-page flash, at the cost of a few seconds with no window/visual feedback at all on cold start.
18. [`public/` Never Bundled (Bug 4), Then A Reproducible Cross-Compile Build Failure](milestones/M2_018_public-dir-unbundled-and-resource-lib-permission-denied.md) — **VERIFIED**. Finds the real cause of the recurring `{"ok":false,"error":"not found"}` the user kept seeing: `public/` (the entire frontend) was never added to `bundle.resources`, so `GET /` itself 404'd this whole time — every prior round of verification only checked `/health`, never the actual page. Fixed in config. The from-source rebuild meant to bake this in properly then hit a separate, reproducible `Permission denied (os error 13)` writing `resource.lib`, four times in a row, surviving process cleanup, direct file deletion, and a full intermediate-build-cache wipe — leading (unconfirmed) suspect is Windows Defender real-time scanning a freshly-written build artifact on the Windows-mounted drive. Mitigation: build from WSL's own native filesystem (`~/vf-build-tmp`) instead of `/mnt/d/...`, avoiding any change to security settings — succeeded on the first attempt, copied back into the repo's normal build path, and live-verified for real this time: `GET /`, `/dev/app.js`, `/dev/styles.css` all `200`, `/health` → `ok:true`.
19. [Vbee Preview Via Session Token And Direct API](milestones/M2_019_direct-api-preview-instead-of-keystrokes.md) — replaces Playwright `pressSequentially` typing with the M2_009 page-context API path: keep the headed studio session to capture the bearer from bootstrap `Authorization` headers, then `page.evaluate()` the demo synthesis WebSocket (`INIT` / `SYNTHESIS`) so Linux does not have to type each character. Token never crosses into Node. 47/47 unit tests pass. Live SYNTHESIS payload field set is still unverified.
20. [Vbee Preview Direct API Test Report](milestones/M2_019_vbee-preview-test-report.md) — syntax + `npm run test:m2` for the adapter rewrite; live Vbee job not run.
21. [Windows-Native Vbee Direct API Driver](milestones/M2_020_windows-direct-api-driver.md) — live probe only. Production worker is still the Linux Gateway adapter (M2_019). Today's logged-in Brave is on Windows, so `scripts/vbee-windows-direct-api.bat` attaches there to prove token capture + SYNTHESIS before the Linux worker box exists. 6/6 CLI unit tests pass; live Brave attach is human-gated.
22. [Windows Direct API Driver Test Report](milestones/M2_020_vbee-windows-direct-api-test-report.md) — parse/guard/redaction tests; WSL `--probe` exits 2.
23. [Queue And Assets CSV Export](milestones/M2_021_csv-export.md) — `GET /api/queue.csv` and `GET /api/assets.csv` (RFC 4180, UTF-8 BOM, full table not the UI 100-row cap, `.tmp` assets hidden), plus Export CSV buttons on the Queue and Assets tabs. Download goes through Gateway HTTP with Bearer auth; `?token=` stays audio-only.
24. [CSV Export Test Report](milestones/M2_021_csv-export-test-report.md) — 57/57 `npm run test:m2`; encoder, export-list, and route auth cases.
25. [Browser Session Persistence — Tier 2](milestones/M2_022_browser-session-persistence-tier2.md) — giải pháp "mỗi lần restart Brave là mất login Vbee" (research đầy đủ + nguồn chính thức ghi ở `.local/VBEE_SESSION_RESEARCH.md`). Implement Tier 2 trong `scripts/browser-lifecycle.mjs`: thêm `--restore-last-session` (session cookie Vbee sống qua restart), `stop` đóng MỀM (CDP `Browser.close` → fallback `taskkill /T` không `/F` / SIGTERM, bỏ force-kill), thêm lệnh `close` đóng mềm cho browser external/remote (homelab thay `pkill`). Không thêm dependency (WebSocket native Node ≥22); Tauri hưởng vì route cùng script. Kèm plan Tier 3 (Playwright `launch_persistent_context`) và Tier 4 (direct-API token reuse).
26. [Browser Session Persistence — Test Report](milestones/M2_022_browser-session-persistence-test-report.md) — live-verified: `close` CDP trên Brave thật (port 9222 đóng), start→status→stop managed `closeMethod:"cdp"`, 24/24 `test:m2`, context-check OK. Residual: chưa test giữ-session qua restart với account thật trên homelab (human-gated); `build` context treo >300s trên /mnt/d.
27. [Voice Catalog Hybrid Picker](milestones/M2_023_voice-catalog-hybrid-picker.md) — implement bước 1–3 của design/12: chọn giọng API-first + hybrid (giọng Vbee + cá nhân/cộng đồng). Module mới `gateway/src/vbee/catalog/voice-catalog.js` (normalize/classify ownership/merge/extract, auto-detect field), adapter contract mở rộng `listVoices()` (FakeVbeeAdapter seed; VbeePreviewAdapter fetch catalog trong `page.evaluate` với token không rời page context M2_019, cache TTL 5p, degrade-don't-throw), `GET /api/voices`, UI datalist picker + status + `resolveVoiceCode` (nhập code hoặc tên). Env: `VBEE_VOICES_URL`, `VBEE_VOICES_ARRAY_FIELD`. 77/77 `test:m2`.
28. [Voice Catalog Hybrid Picker — Test Report](milestones/M2_023_voice-catalog-test-report.md) — 77/77 pass; +20 tests (catalog 11, fake-vbee 1, vbee-preview 5, routes 3); syntax + context check OK. Residual: endpoint studio chưa capture live (human-gated), live accept `voice_code`/`speed` chưa verify, UI chưa smoke trên browser thật.
