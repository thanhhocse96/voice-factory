...
```mermaid
flowchart TD
  Ask["User asks: 'Huớng dẫn tôi kết xuất thành phần mềm'"] --> Scope["AskUserQuestion: which meaning - user picks 'installer (.exe/.msi) for another machine'"]
  Scope --> Audit["Audit current state: no Windows-native Rust toolchain, bundle.active:false, project_root() hardcodes CARGO_MANIFEST_DIR (non-relocatable), Gateway/Brave never bundled"]
  Audit --> RustQ["AskUserQuestion: MSVC vs GNU linker, bundle Node or require it preinstalled"]
  RustQ --> Pushback["User: 'Tôi có cài trong wsl mà?' - already has Rust, just not for Windows target"]
  Pushback --> CrossCompile["Pivot: cross-compile x86_64-pc-windows-gnu FROM WSL via mingw-w64 + NSIS (makensis) — officially 'experimental' per Tauri docs, but avoids Windows-side toolchain install"]
  CrossCompile --> Config["tauri.conf.json: bundle.active true, targets [nsis], bundle.resources mapping gateway/scripts/server.js/package.json"]
  Config --> IconBug["tauri icon fails: existing 32x32 icon.png is corrupt (invalid deflate stream)"]
  IconBug --> IconFix["Hand-build a valid 1024x1024 PNG via Node's zlib, re-run tauri icon successfully"]
  IconFix --> RootFix1["Fix project_root(): CARGO_MANIFEST_DIR only for debug; release falls back to exe runtime directory"]
  RootFix1 --> Build1["First successful cross-compiled build + NSIS installer produced"]
  Build1 --> Bug1["LIVE: user reports '127.0.0.1 refused to connect' after install/run"]
  Bug1 --> Diag1["Diagnosed: bundle.resources lands beside exe (not under resources/) for NSIS — first project_root() fix guessed wrong"]
  Diag1 --> Fix1["Fix: check both locations, use whichever has marker file"]
  Fix1 --> Build2["Rebuild, retest — STILL refused"]
  Build2 --> Diag2["Found unrelated bug: gateway-lifecycle.mjs spawns 'npm' with shell:false — Windows resolves npm to npm.cmd, spawn() fails silently"]
  Diag2 --> Fix2["Fix: spawn(process.execPath, [...]) — remove npm dependency"]
  Fix2 --> Verify["LIVE: Gateway + Brave start, /health ok:true, browserCdp available — verified via curl"]
  Verify --> FinalBuild["Official rebuild with both fixes baked in"]
  FinalBuild --> Ux["User raises UX issue: window races Gateway startup on cold launch"]
  Ux --> Defer["Confirmed: window navigates before start_gateway() returns — deferred, two follow-up plan docs"]
```
